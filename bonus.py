...
def process_messages(client, messages, tools=None, available_functions=None):
    ...
    # Step 3: If the model requested a tool call, invoke the tool
    if response_message.tool_calls:
        for tool_call in response_message.tool_calls:
        ...
        # Step 6: Send a follow-up request with the tool response
...
def run_conversation(client, system_message="You are a helpful weather assistant."):
    ...
    while True:
        ...
        messages = process_messages(client, messages, tools=weather_tools, available_functions=available_functions)
...
# Add calculator tool to weather tools and available functions
cot_tools = weather_tools + [calculator_tool]
available_functions["calculator"] = calculator

# System message for Chain of Thought agent
cot_system_message = (
...
# Define ReAct agent tools by combining all previous tools with the search tool.
react_tools = cot_tools + [search_tool]
available_functions["web_search"] = web_search

react_system_message = (
    "You are a helpful weather and information assistant that uses the ReAct (Reasoning and Acting) approach to solve problems.\n\n"
...
def run_comparative_evaluation(client):
    """
    Run the comparative evaluation by processing a single user query with all three agent types.
    """
    ...
    # Define system messages for the three agent types
    system_messages = {
